...
Since my goal was to minimize compute costs, I chose the EfficientNet architecture. <Sidenote>The [original Efficientnet paper](https://arxiv.org/abs/1905.11946) was published by Tan and Le in 2019. The EfficientNet architecture uses a compound scaling method that uniformly scales all architectural dimensions of the CNN (depth/width/resolution).</Sidenote> The big benefit of the Efficient is that its optimized for performance/compute cost. At the time of publication, their largest model, the *EfficientNetB7* was deemed "8.4x smaller and 6.1x faster on inference than the best existing ConvNet"<Sidenote>[Tan & Lee, 2019]()</Sidenote>. Here's a figure from the paper:
...
path = os.path.join(DATA_DIR, 'cataract').listdir()[0] # get the first image in the cataract folder
img = Image.open(path)

img.show() # sometimes notebooks can get funky with this, you can try the following line instead if it doesn't work
# print(img) 
```
...
<Callout type='warning'>
    If you are NOT using a GPU runtime (`!nvidia-smi` should show a GPU), this will take a looooooong time (hours or days) and might never finish. If you're on a CPU, I would recommend restarting the runtime and selecting a GPU runtime, before working back to this point (you'll need to re-run all previous cells).
</Callout>
